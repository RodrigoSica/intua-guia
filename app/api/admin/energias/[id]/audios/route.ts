...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const r2Key = `e/${energiaId}/${escopo}/audio-${ordem}`;
    ...
    await db.insert(energiaAudios).values({ id: audioId, energiaId, secaoId, ordem, r2Key });

    return Response.json({ audio: { id: audioId, energiaId, secaoId, ordem, r2Key } }, { status: 201 });
  } catch (error) {
